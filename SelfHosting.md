...
`oxen-server` continues to read the legacy form on every load, so an upgrade does not force a migration and no admin action is required. The legacy `type` key is ignored, leaving `kind` at its `local` default, and `[storage.settings] path` is promoted to `versions_path`.

...
pub fn new_for_url_with_bearer_token(url: &str, bearer_token: &str) -> Result<Client, OxenError> {
    let (_scheme, host) = get_scheme_and_host_from_url(url)?;
...
fn builder_for_host_with_bearer_token(
    host: &str,
    bearer_token: &str,
    should_add_user_agent: bool,
    ...
    log::debug!("Setting bearer token for host: {}", host);

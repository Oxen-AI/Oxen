...
impl WebhookDB {
    ...
    pub fn update_notification_stats(&self, webhook_id: &str) -> Result<(), OxenError> {
        if let Some(mut webhook) =
            str_json_db::get::<MultiThreaded, &str, Webhook>(&self.db, webhook_id)?
        {

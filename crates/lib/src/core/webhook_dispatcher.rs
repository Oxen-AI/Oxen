...
impl WebhookDispatcher {
            ...
            rt.block_on(async move {
                ...
                match notifier.notify_path_changed(&repo_path, changed_path).await {
                    Ok(count) => {
                        if count > 0 {
                            log::info!(
                                "Sent {} webhook notifications for commit {}",
                                count,
                                commit_id
                            );
                            println!(
                                "🔔 {} webhook callbacks done for commit {}",
                                count, commit_id
                            );
                        }
                    }
                    Err(e) => {
                        log::error!(
                            "Failed to send webhook notifications for commit {}: {}",
                            commit_id,
                            e
                        );
                    }
    ...
    async fn dispatch_to_queue(
        ...
        let event_json = serde_json::to_string(&event).map_err(|e| {
            OxenError::basic_str(format!("Failed to serialize webhook event: {}", e))
        })?;
        ...
        log::info!(
            "Queued webhook event for commit {} in {}",
            commit.id,
            queue_path.display()
        );
        Ok(())
    ...
    pub fn save_config(&self, repo: &LocalRepository) -> Result<(), OxenError> {
        ...
        let config_json = serde_json::to_string_pretty(&self.config).map_err(|e| {
            OxenError::basic_str(format!("Failed to serialize webhook config: {}", e))
        })?;

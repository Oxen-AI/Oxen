...
impl WebhookNotifier {
    ...
    pub async fn notify_path_changed(
        ...
        for webhook in &webhooks {
            ...
            if let Some(last_notified) = webhook.last_notified {
                ...
                if let Ok(elapsed) = now.duration_since(last_notified)
                    && elapsed.as_secs() < self.rate_limit_seconds
                {
                    continue;
                }
            }
            ...
            match self.send_webhook_notification(webhook, &notification).await {
                Ok(_) => {
                    ...
                    let counter = self
                        .stats
                        .entry("notifications_sent".to_string())
                        .or_insert(0);
                    *counter += 1;
                }
                Err(err) => {
                    ...
                    match webhook_db.record_notification_failure(&webhook.id) {
                        Ok(was_removed) => {
                            if was_removed {
                                ...
                                let counter = self
                                    .stats
                                    .entry("webhooks_auto_removed".to_string())
                                    .or_insert(0);
                                *counter += 1;
                                log::info!(
                                    "Auto-removed webhook {} after consecutive failures",
                                    webhook.id
                                );
                            }
                        }
                    ...
                    let counter = self
                        .stats
                        .entry("notifications_failed".to_string())
                        .or_insert(0);
                    *counter += 1;
                }
            }
        ...
        if removed_count > 0 {
            log::info!(
                "Auto-removed {} failing webhooks for path {}",
                removed_count,
                changed_path
            );
        }
    ...
    pub async fn cleanup_old_webhooks(
        &self,
        repo_path: &Path,
        max_age_days: u64,
    ) -> Result<usize, OxenError> {

...
/// obtain a `MerkleReader` — [`LocalRepository::merkle_store`] — returns a
/// type whose `Error` is pinned to [`OxenError`] via an adapter wrapper, so
/// callers need no `where` clauses and can use `?` directly.

...
///
/// Object-safe: callers can store this as `Box<dyn MerkleWriter + '_>` or
/// `&dyn MerkleWriter`. The error type is fixed to [`OxenError`] so the trait
/// stays object-safe (no associated type that callers would need to bind);
/// backends with their own native error types convert at the trait boundary.

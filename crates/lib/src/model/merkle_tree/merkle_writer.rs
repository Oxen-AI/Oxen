...
pub trait MerkleWriter: Send + Sync {
    ...
    ///
    /// To ensure that changes are persisted, callers must call [`finish`] on the returned session.
    /// Correct use of the returned session is to create a [`NodeWriteSession`] for each node to be
    /// written, and then call [`finish`] on that session when complete with the node.
    fn begin(
        &self,
...
/// [`MerkleWriteSession`] when comitting repository changes. From this one write session,
/// callers will create multiple [`NodeWriteSession`]s to write the nodes they need to store.
/// Each [`NodeWriteSession`] must have its [`finish`] called to finialize the written node
...
///
/// Peristance and eagerness of writes are implementation details. Implementations may choose
/// to buffer writes or write immediately to the store when [`create_node`] and [`add_child`]
/// are called. The invairant is that [`finish`] must be called to **ensure** that writes are
...
///
/// Implementations may buffer the `node` and `children` information in memory or chose to write
/// the data to the store eagerly. However, if [`finish`] is called and returns `Ok`, then the
/// guarentee is that all node and child information must be persisted to the store.

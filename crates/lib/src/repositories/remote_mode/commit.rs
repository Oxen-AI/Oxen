...
mod tests {
    ...
    #[tokio::test]
    async fn test_remote_mode_commit_file(#[case] kind: MerkleStoreKind) -> Result<(), OxenError> {
        test::run_remote_repo_test_bounding_box_csv_pushed(
            ...
            |_local_repo, remote_repo| async move {
                ...
                test::run_empty_dir_test_async(|dir| async move {
                    ...
                    let file_path =
                        test::add_txt_file_to_dir(&cloned_repo.path, "new file contents")?;
    ...
    #[case::lmdb(MerkleStoreKind::Lmdb)]
    #[tokio::test]
    async fn test_remote_mode_commit_several_times(
            ...
            |_local_repo, remote_repo| async move {
                ...
                test::run_empty_dir_test_async(|dir| async move {
                    ...
                    for i in 1..=4 {
                        ...
                        let status_opts = StagedDataOpts::from_paths_remote_mode(
                            std::slice::from_ref(&cloned_repo.path),
                        );
                        let status = repositories::remote_mode::status(
                            &cloned_repo,
                            &remote_repo,
                    ...
                    let history = repositories::commits::list_between(
                        &cloned_repo,
                        &base_commit,
                        &head_commit,
                    )?;
    ...
    #[case::lmdb(MerkleStoreKind::Lmdb)]
    #[tokio::test]
    async fn test_remote_mode_add_and_commit_downloaded_dir(
        #[case] kind: MerkleStoreKind,
    ) -> Result<(), OxenError> {
        test::run_remote_repo_test_bounding_box_csv_pushed(
            kind,
    ...
    #[case::lmdb(MerkleStoreKind::Lmdb)]
    #[tokio::test]
    async fn test_remote_mode_cannot_commit_without_staged_files(
        #[case] kind: MerkleStoreKind,
    ) -> Result<(), OxenError> {
    ...
    #[case::lmdb(MerkleStoreKind::Lmdb)]
    #[tokio::test]
    async fn test_remote_mode_commit_removed_file(
        #[case] kind: MerkleStoreKind,
    ) -> Result<(), OxenError> {
        test::run_remote_repo_test_bounding_box_csv_pushed(
            kind,
            |_local_repo, remote_repo| async move {
                ...
                test::run_empty_dir_test_async(|dir| async move {
                    ...
                    // Commit the deletion
                    let commit_body =
                        NewCommitBody::from_config(&UserConfig::get()?, "Second Message");
                    repositories::remote_mode::commit(&cloned_repo, &commit_body).await?;
    ...
    #[case::lmdb(MerkleStoreKind::Lmdb)]
    #[tokio::test]
    async fn test_remote_mode_commit_removed_dir(
        #[case] kind: MerkleStoreKind,
    ) -> Result<(), OxenError> {
        test::run_remote_repo_test_bounding_box_csv_pushed(
            kind,
    ...
    #[case::lmdb(MerkleStoreKind::Lmdb)]
    #[tokio::test]
    async fn test_remote_mode_commit_invalid_parquet_file(
        #[case] kind: MerkleStoreKind,
    ) -> Result<(), OxenError> {
        test::run_remote_repo_test_bounding_box_csv_pushed(
            kind,

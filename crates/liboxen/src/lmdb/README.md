...
This directory is the reusable, **low-level LMDB layer**. Application code should **not** use it directly. Instead, write a thin per-domain *store wrapper* that names its env's location and its database, and chooses its own key encoding and value framing. This guide walks through writing one. (For the layer's internal design, read the module doc in `../lmdb.rs` and the per-file docs; this README is the implementor's how-to.)

## The shape, in one breath

A store is **one LMDB env** (one logical store per env) holding **one named database**, implementing the **`LmdbStore`** trait, which opens the env on first use through **`open_shared_env`** (the layer's process-global registry, so a path is opened at most once) and gives the store its lifecycle (`read` / `write` / `snapshot_to`) for free, exposing **inherent domain methods** written over `self.read` / `self.write`, and wrapped in **`spawn_blocking`** at the async edge (the layer is synchronous).
...
impl RefsStore {
    /// The refs store for the repo at `dir`. Its env opens on the first read or write.
    pub fn new(dir: &Path) -> Self {
        RefsStore {
            lmdb: LmdbSlot::new(dir.to_path_buf()),
        }
    }
...
impl LmdbStore for RefsStore {
    // This store's fixed budget (step 1).
    const LMDB_MAP_SIZE: ByteSize = ByteSize::mib(256);
    const LMDB_DB_NAME: &'static str = "refs";
...
Keep one logical store per env. Do **not** lump several unrelated stores into a single env: LMDB allows **one writer per env**, so a shared env serializes every write through one lock and throws away cross-store write concurrency. (Consolidating *many instances of the same store* — e.g. per-resource directories — into one env with composite keys is fine and encouraged; that's different from mixing distinct stores.)

## Step 3 — let the trait open the env

`LmdbStore` opens the env at the directory your `LmdbSlot` was built with, reserving `LMDB_MAP_SIZE`, on the store's first `read`, `write`, or `snapshot_to`, and caches the opened handles in that slot. Until then nothing is opened and nothing is created on disk, so a store that is constructed and never used costs nothing, and its constructor is infallible.

The trait opens through `open_shared_env(dir, map_size)` (the raw open primitive is module-private precisely so this can't be bypassed). It returns a shared `Arc<LmdbEnv>` from the layer's **process-global** registry, enforcing **at most one live env per canonical path** and deduping overlapping opens — so a store does **not** stand up its own registry; it just names its map size. The registry is **weak-retention**: it does not keep the env alive — *your store does*, by holding the handles in its slot. When the last `Arc` drops, the env closes; a later `open_shared_env` reopens it cheaply (an idle env's hot pages stay warm in the OS page cache). The map size is consulted only on an actual open; on a shared hit it is ignored. (A higher layer — a per-repo cache — may eventually own env handles instead.)

## Step 4 — name your database

The trait opens the database named by `LMDB_DB_NAME` alongside the env, through `open_db`, which runs the write txn heed requires for opening a database. Every env holds exactly one database. A store needing several would first need the env's `max_dbs` raised and the slot's handles extended to hold them, opening them together inside one `with_write_txn` closure with the lower primitive `LmdbDb::open(&env, txn, "name")` rather than paying a separate write txn per database. That is not present yet, so add it when a store that needs it arrives. A `LmdbDb` is a raw-bytes key / opaque-bytes value accessor, and the trait holds it in the handles slot for the store's lifetime. Keys are plain `&[u8]` and values are opaque bytes — **you** decide how to encode keys and frame values (including any magic/version bytes). LMDB orders keys by lexicographic byte comparison; that is correct for point lookups and for byte/string-ordered scans, but a store needing *numeric*-ordered range scans over a multi-byte integer would first need a big-endian key codec added to the layer (not present yet — add it when a verified consumer needs it).

## Step 5 — implement `LmdbStore` for the lifecycle

Name your `LMDB_MAP_SIZE` and `LMDB_DB_NAME`, hand over your slot through `lmdb_slot`, and you inherit:
...
The store (its `LmdbSlot`) is `Send + Sync`, so wrap it in `Arc` and clone it into the closure.
...
- **Always open through `LmdbStore`.** The open primitives are private to the layer, so a store cannot construct a second env for the same path, which would break the one-live-env-per-path invariant the shared registry exists to hold.
...
- `lmdb_env.rs` — `LmdbEnv` (heed `Env<WithoutTls>`), the open primitive, and `copy_lmdb_env_to_dir` (backing `snapshot_to`).
...
- `store.rs` — `LmdbStore`: the default-method lifecycle trait you implement, and `LmdbSlot`, where your env lives and the opened env and database it caches.

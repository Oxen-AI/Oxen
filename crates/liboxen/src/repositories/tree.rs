...
pub fn get_root(
    ...
    CommitMerkleTreeLatest::root_without_children(repo, commit)
}
...
pub fn get_root_with_children(
    ...
    CommitMerkleTreeLatest::root_with_children(repo, commit)
}
...
pub fn get_root_with_children_and_node_hashes(
    ...
    CommitMerkleTreeLatest::root_with_children_and_node_hashes(
        repo,
        commit,
        base_hashes,
        unique_hashes,
...
pub fn get_root_with_children_and_partial_nodes(
    ...
    CommitMerkleTreeLatest::root_with_children_and_partial_nodes(
        repo,
        commit,
        base_hashes,
        unique_hashes,
...
pub fn get_node_by_id(
    ...
    CommitMerkleTreeLatest::read_node(repo, hash, load_recursive)
}
...
pub fn get_node_by_path_with_children(
    ...
    let load_recursive = true;
    let node = CommitMerkleTreeLatest::read_from_path(repo, commit, path, load_recursive)?;
...
pub fn get_subtree_by_depth_with_unique_children(
    ...
    CommitMerkleTreeLatest::read_depth_from_path_and_collect_hashes(
        repo,
        commit,
        path,
        base_hashes,
...
pub fn list_nodes_from_paths(
    ...
    CommitMerkleTreeLatest::read_nodes(repo, commit, paths)
}
...
/// Unpack a tar-gz wire stream into the repo's Merkle node `store`.
///
...
/// - `overwrite_existing == false` leaves existing nodes untouched; matches
...
fn extract_tar_under<R: Read>(
    ...
    for entry in entries {
        ...
        // Resolve to the canonical `<oxen_hidden>/tree/nodes/...` form so one classifier handles
        // both wire layouts: server-style entries already carry `tree/nodes/...`; legacy
        // client-push entries begin at `{prefix}/{suffix}/...` and get the prefix prepended.
        let entry_path = if path.starts_with(&tree_nodes_prefix) {
            oxen_hidden.join(&path)
        } else {
            oxen_hidden.join(&tree_nodes_prefix).join(&path)
        };

        // Validate the entry's shape; this propagates the structured errors for malformed
        ...
        // A regular file under `tree/nodes` is a `node` or `children` leaf, and its hash is the
        // `{prefix}/{suffix}` parent directory.
        let Some(is_node) = entry_path
            .file_name()
            .and_then(|n| n.to_str())
            .map(|n| n == NODE_FILE)
        else {
        ...
        let hash = match entry_path.parent() {
            Some(parent) => extract_hash_from_entry_path(parent, oxen_hidden)?,
            None => None,
        };

...
- **Streamed IO through `AsyncRead` / `AsyncWrite`** still gets the 10 MB `BufWriter` treatment from the project AGENTS.md, and an explicit `flush().await?` before any downstream `sync_all` / rename / checksum.

...
pub async fn completed(
    req: HttpRequest,
    body: Option<web::Json<PushCompleteBody>>,
            ...
            match repositories::get_by_namespace_and_name(&app_data.path, namespace, repo_name) {
                Ok(Some(repo)) => {
                    match repositories::commits::get_by_id(&repo, &push_data.branch.commit_id) {
                        Ok(Some(commit)) => match WebhookDispatcher::from_repo(&repo) {
                            Ok(dispatcher) => {
                                ...
                                tokio::spawn(async move {
                                        ...
                                        .await
                                    {
                                        Ok(_) => {
                                            log::debug!(
                                                "Webhook notifications dispatched successfully for push action"
                                            );
                                        }
                                        Err(e) => {
                                            log::error!(
                                                "Failed to dispatch webhook notifications for push action: {e}"
                                            );
                                        }
                                    }
                                });
                            ...
                            Err(e) => {
                                log::error!(
                                    "Failed to create webhook dispatcher for push action: {e}"
                                );
                            }
                        },
                        Ok(None) => {
                            log::error!(
                                "Could not find commit {} for push webhook",
                                push_data.branch.commit_id
                            );
                        }
                        Err(e) => {
                            log::error!(
                                "Error finding commit {} for push webhook: {}",
                                push_data.branch.commit_id,
                                e
                            );
                        }
                ...
                Ok(None) => {
                    log::error!(
                        "Could not find repository {namespace}/{repo_name} for push webhook"
                    );
                }
                Err(e) => {
                    log::error!(
                        "Error finding repository {namespace}/{repo_name} for push webhook: {e}"
                    );
                }
            }
        }

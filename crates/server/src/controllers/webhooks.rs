...
fn get_authenticated_user(
    req: &HttpRequest,
    app_data_path: &std::path::Path,
) -> Result<Option<User>, OxenHttpError> {
        ...
        if let Ok(auth_str) = auth_value.to_str() {
            if let Some(token) = auth_str.strip_prefix("Bearer ") {
                ...
                match AccessKeyManager::new_read_only(app_data_path) {
                    Ok(keygen) => match keygen.get_claim(token) {
                        Ok(Some(claim)) => {
                        ...
                        Ok(None) => {
                            log::debug!(
                                "🔑 ❌ Token validation returned None for webhook operation"
                            );
                        }
                        Err(e) => {
                            log::debug!(
                                "🔑 ❌ Token validation error for webhook operation: {:?}",
                                e
                            );
                        }
                    },
                    Err(err) => {
                        log::debug!(
                            "🔑 ❌ AccessKeyManager creation failed for webhook operation: {:?}",
                            err
                        );
                    }
                }
                ...
                log::debug!(
                    "🔑 ❌ Authorization header does not start with 'Bearer ' for webhook operation"
                );
            }
        } else {
            log::debug!(
                "🔑 ❌ Could not parse authorization header as string for webhook operation"
            );
        }
    } else {
...
pub async fn add_webhook(
    ...
    std::fs::create_dir_all(&webhook_db_path).map_err(|e| {
        actix_web::error::ErrorInternalServerError(format!(
            "Failed to create webhook directory: {}",
            e
        ))
    })?;
    ...
    let webhook_db = WebhookDB::new(&webhook_db_path).map_err(|e| {
        actix_web::error::ErrorInternalServerError(format!(
            "Failed to open webhook database: {}",
            e
        ))
    })?;
...
pub async fn list_webhooks(req: HttpRequest) -> actix_web::Result<HttpResponse, OxenHttpError> {
    let app_data = app_data(&req)?;
...
pub async fn webhook_stats(req: HttpRequest) -> actix_web::Result<HttpResponse, OxenHttpError> {
    let app_data = app_data(&req)?;
...
pub async fn remove_webhook(
    ...
    match webhook_db.remove_webhook(webhook_id) {
        Ok(true) => {
            Ok(HttpResponse::Ok().json(json!({"success": true, "message": "Webhook removed"})))
        }

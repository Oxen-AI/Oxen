...
use actix_web::{App, HttpResponse, Result as ActixResult, test, web};
...
use crate::test::{cleanup_sync_dir, create_local_repo, get_sync_dir};
...
impl MockAuth {
    ...
    fn validate_bearer_token(&self, token: &str) -> Result<bool, String> {
        self.operations_log
            .lock()
            .unwrap()
            .push(format!("validate_bearer_token:{}", token));
...
impl MockRepository {
    ...
    fn get_current_revision(&self) -> Result<Option<String>, String> {
        self.operations_log
            .lock()
            .unwrap()
            .push("get_current_revision".to_string());
...
async fn test_add_webhook_http_layer_authentication_required() {
    ...
    assert!(
        operations_log.is_empty(),
        "Database operations should not be called without authentication"
    );
}
...
async fn test_add_webhook_http_layer_invalid_revision() {
    ...
    assert_eq!(
        resp.status(),
        400,
        "Should return 400 Bad Request for revision mismatch"
    );

    let response_body: Value = test::read_body_json(resp).await;
    assert_eq!(
        response_body["error"], "no",
        "Should return 'no' error for revision mismatch"
    );
    ...
    assert!(
        db_operations.is_empty(),
        "Database should not be called for invalid revision"
    );
}
...
async fn test_add_webhook_http_layer_successful_flow() {
    ...
    assert_eq!(
        resp.status(),
        200,
        "Should return 200 OK for successful webhook creation"
    );

    let response_body: Value = test::read_body_json(resp).await;
    assert!(
        response_body["id"].is_string(),
        "Response should contain webhook ID"
    );
    ...
    assert!(
        db_operations
            .iter()
            .any(|op| op.starts_with("add_webhook:")),
...
async fn test_list_webhooks_http_layer_authentication_required() {
    ...
    assert_eq!(
        resp.status(),
        401,
        "Should require authentication for listing webhooks"
    );
    ...
    assert!(
        operations_log.is_empty(),
        "No business logic should be called without auth"
    );
}
...
async fn test_list_webhooks_http_layer_successful_response_formatting() {
    ...
    mock_db
        .add_webhook("webhook-123".to_string(), test_webhook)
    ...
    assert_eq!(
        resp.status(),
        200,
        "Should return 200 OK for successful list"
    );

    let response_body: Value = test::read_body_json(resp).await;
    assert!(
        response_body["webhooks"].is_array(),
        "Response should contain webhooks array"
    );
    ...
    let operations_log = mock_db.get_operations_log();
    assert!(
        operations_log.contains(&"list_webhooks".to_string()),

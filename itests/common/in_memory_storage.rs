...
impl VersionStore for InMemoryVersionStore {
    ...
    fn store_version_from_path(&self, hash: &str, file_path: &Path) -> Result<(), OxenError> {
        ...
        self.store_version(hash, &data)
    }

    fn store_version_from_reader(&self, hash: &str, reader: &mut dyn Read) -> Result<(), OxenError> {
    ...
    fn combine_version_chunks(&self, hash: &str, _cleanup: bool) -> Result<PathBuf, OxenError> {
        ...
        self.store_version(hash, &combined_data)?;
...
mod tests {
    ...
    fn test_basic_storage_operations() {
        ...
        assert!(store.store_version(hash, data).is_ok());
    ...
    fn test_chunk_operations() {
        ...
        // Store chunks
        assert!(store.store_version_chunk(hash, 0, b"Hello, ").is_ok());
        assert!(store.store_version_chunk(hash, 1, b"world!").is_ok());
    ...
    fn test_clear_functionality() {
        ...
        // Add some data
        store.store_version("hash1", b"data1").unwrap();
        store.store_version("hash2", b"data2").unwrap();

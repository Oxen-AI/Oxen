...
impl TestPortAllocator {
    ...
    pub fn lease_port(&self) -> Result<PortLease, String> {
        ...
        Err(format!("No available ports in range {}-{}", self.port_range_start, self.port_range_end))
    }
    ...
    fn test_port_allocation_thread_safety() {
        ...
        for i in 0..5 {
            ...
            let handle = thread::spawn(move || {
                let lease = allocator_clone.lease_port()
        ...
        assert_eq!(allocator.allocated_ports().len(), 0, "All ports should be freed");
    }

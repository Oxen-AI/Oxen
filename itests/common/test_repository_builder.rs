...
impl TestRepositoryBuilder {
    ...
    pub fn build(self) -> Result<TestRepository, OxenError> {
        ...
        let repo = if self.use_memory_storage {
            self.init_repo_with_in_memory_storage(&repo_dir)?
        } else {
    ...
    fn init_repo_with_in_memory_storage(&self, repo_dir: &Path) -> Result<LocalRepository, OxenError> {
...
impl TestRepository {
    ...
    pub fn add_file(&mut self, path: &str, content: &str) -> Result<(), OxenError> {
        ...
        repositories::add(&self.repo, &full_path)?;
        self.files.insert(path.to_string(), content.to_string());
...
mod tests {
    ...
    #[test]
    fn test_basic_repository_creation() {
    ...
    #[test]
    fn test_add_file_after_creation() {
        ...
        // Add another file
        test_repo.add_file("added.txt", "Added content").unwrap();
        assert_eq!(test_repo.files().len(), 2);

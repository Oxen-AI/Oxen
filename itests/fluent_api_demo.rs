...
async fn demonstrate_fluent_api() {
    ...
    assert_eq!(store.file_content("data.csv").unwrap(), "id,name\n1,Alice\n2,Bob");
    assert_eq!(store.file_content("config.json").unwrap(), r#"{"version": "1.0"}"#);
...
async fn demonstrate_post_creation_operations() {
    ...
    test_repo.add_file("nested/file.txt", "Nested content").unwrap();

...
mod tests {
    ...
    async fn test_compare_commits() -> Result<(), OxenError> {
        oxen_test::run_empty_remote_repo_test(|mut local_repo, remote_repo| async move {
    ...
    async fn test_compare_change_at_root_dir_tree() -> Result<(), OxenError> {
        oxen_test::run_one_commit_sync_repo_test(|mut local_repo, remote_repo| async move {
            ...
            oxen_test::write_txt_file_to_path(full_path, "i am the contents of test_me_out.txt")?;
            repositories::add(&local_repo, &local_repo.path).await?;
    ...
    async fn test_compare_change_tabular_at_root_dir_tree() -> Result<(), OxenError> {
        oxen_test::run_one_commit_sync_repo_test(|mut local_repo, remote_repo| async move {
            ...
            oxen_test::write_txt_file_to_path(full_path, "image,label\n1,2\n3,4\n5,6")?;
            repositories::add(&local_repo, &local_repo.path).await?;
    ...
    async fn test_compare_nested_dir_tree() -> Result<(), OxenError> {
        oxen_test::run_empty_remote_repo_test(|mut local_repo, remote_repo| async move {
            ...
            for i in 0..total_dirs {
                ...
                oxen_test::write_txt_file_to_path(full_path, format!("File content {i}"))?;
                repositories::add(&local_repo, &local_repo.path).await?;
    ...
    async fn test_remote_create_compare_get_derived() -> Result<(), OxenError> {
        oxen_test::run_empty_remote_repo_test(|mut local_repo, remote_repo| async move {
            ...
            oxen_test::write_txt_file_to_path(local_repo.path.join(left_path), csv1)?;
            oxen_test::write_txt_file_to_path(local_repo.path.join(right_path), csv2)?;

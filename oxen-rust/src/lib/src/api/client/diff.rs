...
mod tests {
    ...
    async fn test_diff_entries_add_image() -> Result<(), OxenError> {
        oxen_test::run_empty_data_repo_test_no_commits_async(|mut repo| async move {
            // create the images directory
            let images_dir = repo.path.join("images");
            util::fs::create_dir_all(&images_dir)?;

            // Add and commit the first cat
            let test_file = oxen_test::test_img_file_with_name("cat_1.jpg");
            ...
            let test_file = oxen_test::test_img_file_with_name("cat_2.jpg");
            let repo_filepath = images_dir.join(test_file.file_name().unwrap());
            util::fs::copy(&test_file, &repo_filepath)?;
    ...
    async fn test_diff_entries_modify_add_rows_csv() -> Result<(), OxenError> {
        oxen_test::run_empty_data_repo_test_no_commits_async(|mut repo| async move {
            ...
            let test_file = oxen_test::test_csv_file_with_name("llm_fine_tune.csv");
            ...
            let repo_filepath = oxen_test::append_line_txt_file(repo_filepath, "answer the question,what is the color of the sky?,blue,trivia")?;
            let repo_filepath = oxen_test::append_line_txt_file(repo_filepath, "answer the question,what is the color of the ocean?,blue-ish green sometimes,trivia")?;
    ...
    async fn test_diff_entries_modify_add_and_remove_rows_csv() -> Result<(), OxenError> {
        oxen_test::run_empty_data_repo_test_no_commits_async(|mut repo| async move {
            ...
            // Modify and commit the dataframe
            let repo_filepath = oxen_test::write_txt_file_to_path(
                repo_filepath,
...
"#,
    ...
    async fn test_list_diff_entries_cifar_csvs() -> Result<(), OxenError> {
        oxen_test::run_empty_data_repo_test_no_commits_async(|mut repo| async move {
            // Track test.csv file
            let test_file = oxen_test::test_csv_file_with_name("test_cifar_2x9999.csv");
            ...
            let test_file = oxen_test::test_csv_file_with_name("train_cifar_2x50000.csv");
            ...
            let test_file = oxen_test::test_csv_file_with_name("test_cifar_2x10000.csv");
            let repo_filename = "test.csv";
            let repo_filepath = repo.path.join(repo_filename);
            util::fs::copy(&test_file, &repo_filepath)?;
            ...
            // Track train.csv file
            let test_file = oxen_test::test_csv_file_with_name("train_cifar_5x50000.csv");
            let repo_filename = "train.csv";
            let repo_filepath = repo.path.join(repo_filename);
            util::fs::copy(&test_file, &repo_filepath)?;
    ...
    */
    ...
    async fn test_list_diff_entries_adding_images_in_subdirs() -> Result<(), OxenError> {
        oxen_test::run_empty_data_repo_test_no_commits_async(|mut repo| async move {
            ...
            let test_file = oxen_test::test_img_file_with_name("dwight_vince.jpeg");
            let repo_filepath = cats_dir.join(test_file.file_name().unwrap());
            util::fs::copy(&test_file, &repo_filepath)?;

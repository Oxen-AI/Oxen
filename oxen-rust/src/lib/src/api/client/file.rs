...
mod tests {
    ...
    async fn test_update_file() -> Result<(), OxenError> {
        oxen_test::run_remote_repo_test_bounding_box_csv_pushed(
            |local_repo, remote_repo| async move {
                ...
                let file_path = oxen_test::test_img_file();
    ...
    async fn test_update_file_on_empty_repo() -> Result<(), OxenError> {
        oxen_test::run_empty_configured_remote_repo_test(|local_repo, remote_repo| async move {
            let branch_name = "main";
            let directory_name = "test_data";
            let file_path = oxen_test::test_img_file();
            let commit_body = NewCommitBody {
    ...
    async fn test_upload_video_and_get_thumbnail() -> Result<(), OxenError> {
        oxen_test::run_empty_configured_remote_repo_test(|_local_repo, remote_repo| async move {
            let branch_name = DEFAULT_BRANCH_NAME;
            let directory_name = "videos";
            let video_file = oxen_test::test_video_file_with_name("basketball.mp4");

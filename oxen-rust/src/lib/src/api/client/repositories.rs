...
mod tests {
    ...
    async fn test_repo_pre_and_post_clone() -> Result<(), OxenError> {
        oxen_test::run_empty_local_repo_test_async(|local_repo| async move {
            ...
            let mut remote_repo = oxen_test::create_remote_repo(&local_repo).await?;
    ...
    async fn test_create_remote_repository() -> Result<(), OxenError> {
        oxen_test::run_empty_local_repo_test_async(|local_repo| async move {
            ...
            let name = local_repo.dirname();
            let repo_new =
                RepoNew::from_namespace_name_host(namespace, &name, oxen_test::test_host(), None);
            let repository =
    ...
    async fn test_create_remote_repository_with_readme() -> Result<(), OxenError> {
        oxen_test::run_empty_local_repo_test_async(|local_repo| async move {
            ...
            repo_new.host = Some(oxen_test::test_host());

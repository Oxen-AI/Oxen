...
mod tests {
    ...
    async fn test_stage_single_file() -> Result<(), OxenError> {
        oxen_test::run_remote_repo_test_bounding_box_csv_pushed(
            |_local_repo, remote_repo| async move {
                ...
                let path = oxen_test::test_img_file();
        ...
        oxen_test::run_remote_repo_test_bounding_box_csv_pushed(
            |_local_repo, remote_repo| async move {
                ...
                let paths = vec![
                    ...
                    oxen_test::test_img_file_with_name("cole_anthony.jpeg"),
                ];
    ...
    async fn test_create_remote_readme_repo_and_commit_multiple_data_frames(
        ...
        oxen_test::run_remote_created_and_readme_remote_repo_test(|remote_repo| async move {
            ...
            let file_to_post = oxen_test::test_1k_parquet();
            let directory_name = "";
            ...
            let workspace =
                api::client::workspaces::create(&remote_repo, DEFAULT_BRANCH_NAME, &workspace_id)
                    .await?;
            assert_eq!(workspace.id, workspace_id);
            let file_to_post = oxen_test::test_csv_file_with_name("emojis.csv");
            ...
            let workspace =
                api::client::workspaces::create(&remote_repo, DEFAULT_BRANCH_NAME, &workspace_id)
                    .await?;
            assert_eq!(workspace.id, workspace_id);
            let file_to_post = oxen_test::test_invalid_parquet_file();
    ...
    async fn test_commit_staged_single_file_and_pull() -> Result<(), OxenError> {
        oxen_test::run_remote_repo_test_bounding_box_csv_pushed(
            |_local_repo, remote_repo| async move {
                ...
                let file_to_post = oxen_test::test_img_file();
                let directory_name = "data";
                let result = api::client::workspaces::files::upload_single_file(
                    &remote_repo,
                    &workspace_id,
                ...
                oxen_test::run_empty_dir_test_async(|cloned_repo_dir| async move {
                    // Clone repo
                    let opts =
                    ...
                    let path = cloned_repo
                        .path
                        .join(directory_name)
                        .join(oxen_test::test_img_file().file_name().unwrap());
                    assert!(!path.exists());
    ...
    async fn test_commit_schema_on_branch() -> Result<(), OxenError> {
        oxen_test::run_remote_repo_test_bounding_box_csv_pushed(
            |_local_repo, remote_repo| async move {
                ...
                // Post a parquet file
                let path = oxen_test::test_1k_parquet();
                let result = api::client::workspaces::files::upload_single_file(
                    &remote_repo,
                    &workspace_id,
                    directory_name,
    ...
    async fn test_add_file_with_absolute_path() -> Result<(), OxenError> {
        oxen_test::run_remote_repo_test_bounding_box_csv_pushed(
            |_local_repo, remote_repo| async move {
                ...
                let path = oxen_test::test_img_file().canonicalize()?;
                let result = api::client::workspaces::files::add(
                    &remote_repo,
                    &workspace_id,
                    directory_name,
    ...
    async fn test_mv_file() -> Result<(), OxenError> {
        ...
        oxen_test::run_remote_repo_test_all_data_pushed(|remote_repo| async move {

...
mod tests {
    ...
    async fn test_rm_directory() -> Result<(), OxenError> {
        test::run_remote_created_and_readme_remote_repo_test(|remote_repo| async move {
            ...
            let _train_commit = upload_test_file(&remote_repo, &train_fi).await?;

            let test_fi = test_d.join("a_different_file");
            let test_commit = upload_test_file(&remote_repo, &test_fi).await?;
            ...
            let contents =
                api::client::file::get_file(&remote_repo, DEFAULT_BRANCH_NAME, &train_fi).await;
            assert!(
                contents.is_err(),
                "I should be an error but I'm actually: {:?}",
                contents
            );

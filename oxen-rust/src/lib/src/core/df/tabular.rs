...
└──────────┴───────┴───────┴───────┴────────────┘",
    ...
    async fn test_read_json() -> Result<(), OxenError> {
        let df = tabular::read_df_json(oxen_test::test_text_json())?.collect()?;
...
└─────┴───────────┴──────────┘",
    ...
    async fn test_read_jsonl() -> Result<(), OxenError> {
        let df = match task::spawn_blocking(move || -> Result<DataFrame, OxenError> {
            tabular::read_df_jsonl(oxen_test::test_text_jsonl())?
                .collect()
                .map_err(OxenError::from)
        })
        .await?
...
└─────┴───────────┴──────────┘",
    ...
    async fn test_sniff_empty_rows_carriage_return_csv() -> Result<(), OxenError> {
        let opts = DFOpts::empty();
        let df = tabular::read_df(oxen_test::test_csv_empty_rows_carriage_return(), opts).await?;
    ...
    async fn test_sniff_delimiter_tabs() -> Result<(), OxenError> {
        let opts = DFOpts::empty();
        let df = tabular::read_df(oxen_test::test_tabs_csv(), opts).await?;
        assert_eq!(df.width(), 4);
        Ok(())
    }
    ...
    async fn test_sniff_emoji_csv() -> Result<(), OxenError> {
        let opts = DFOpts::empty();
        let df = tabular::read_df(oxen_test::test_emojis(), opts).await?;
    ...
    async fn test_slice_parquet_lazy() -> Result<(), OxenError> {
        ...
        let df = tabular::scan_df_parquet(oxen_test::test_1k_parquet(), 333)?;
    ...
    async fn test_slice_parquet_full_read() -> Result<(), OxenError> {
        let mut opts = DFOpts::empty();
        opts.slice = Some("329..333".to_string());
        let mut df = tabular::read_df(oxen_test::test_1k_parquet(), opts).await?;
        println!("{df:?}");
    ...
    async fn test_parse_file_with_unmatched_quotes() -> Result<(), OxenError> {
        let df = tabular::read_df(oxen_test::test_spam_ham(), DFOpts::empty()).await?;
        assert_eq!(df.width(), 2);

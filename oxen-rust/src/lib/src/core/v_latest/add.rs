...
mod tests {
    ...
    async fn test_add_respects_oxenignore() -> Result<(), OxenError> {
        oxen_test::run_empty_local_repo_test_async(|repo| async move {
            ...
            oxen_test::write_txt_file_to_path(&ignored_path, "This should be ignored")?;
            oxen_test::write_txt_file_to_path(&normal_path, "This should be added")?;

            // Create .oxenignore file with the ignored file pattern
            let oxenignore_path = repo.path.join(".oxenignore");
            oxen_test::write_txt_file_to_path(&oxenignore_path, ignored_file)?;
    ...
    async fn test_add_dot_on_committed_repo() -> Result<(), OxenError> {
        oxen_test::run_empty_local_repo_test_async(|repo| async move {
            ...
            oxen_test::write_txt_file_to_path(&file1_1, "dir1/file1_1")?;
            oxen_test::write_txt_file_to_path(&file1_2, "dir1/file1_2")?;
            oxen_test::write_txt_file_to_path(&file2_1, "dir2/file2_1")?;
            oxen_test::write_txt_file_to_path(&file_root, "file_root")?;
    ...
    async fn test_add_respects_dir_ignore_patterns() -> Result<(), OxenError> {
        oxen_test::run_empty_local_repo_test_async(|repo| async move {
            ...
            let oxenignore_path = repo.path.join(".oxenignore");
            oxen_test::write_txt_file_to_path(&oxenignore_path, format!("{dir_to_ignore}/"))?;

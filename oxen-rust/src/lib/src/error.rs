...
impl OxenError {
    ...
    pub fn remote_repo_not_found(url: &str) -> OxenError {
        let err = format!("Remote repository does not exist {}", url);
    ...
    pub fn file_copy_error(src: &Path, dst: &Path, err: impl Debug) -> OxenError {
        let err = format!(
    ...
    pub fn file_rename_error(src: &Path, dst: &Path, err: impl Debug) -> OxenError {
        let err = format!(

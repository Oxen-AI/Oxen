...
",
    ...
    async fn test_command_add_dot_only_adds_changed_files() -> Result<(), OxenError> {
        oxen_test::run_empty_local_repo_test_async(|repo| async move {
            ...
            // Alter 2 files
            util::fs::remove_file(&hello_file)?;
            oxen_test::modify_txt_file(&world_file, "MODIFIED")?;
    ...
    async fn test_add_all_files_in_sub_dir() -> Result<(), OxenError> {
        oxen_test::run_empty_local_repo_test_async(|repo| async move {
            ...
            let sub_file_1 = oxen_test::add_txt_file_to_dir(&sub_dir, "Hello 1")?;
            let sub_file_2 = oxen_test::add_txt_file_to_dir(&sub_dir, "Hello 2")?;
            let sub_file_3 = oxen_test::add_txt_file_to_dir(&sub_dir, "Hello 3")?;
    ...
    async fn test_cannot_add_if_not_modified() -> Result<(), OxenError> {
        oxen_test::run_empty_local_repo_test_async(|repo| async move {
            ...
            let repo_path = &repo.path;
            let hello_file = oxen_test::add_txt_file_to_dir(repo_path, "Hello World")?;

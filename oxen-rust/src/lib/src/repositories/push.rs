...
mod tests {
    ...
    async fn test_command_push_one_commit() -> Result<(), OxenError> {
        oxen_test::run_training_data_repo_test_no_commits_async(|repo| async {
            let mut repo = repo;
            ...
            let readme_path = oxen_test::write_txt_file_to_path(readme_path, "Ready to train 🏋️‍♂️")?;
    ...
    async fn test_latest_commit_is_computed_properly() -> Result<(), OxenError> {
        oxen_test::run_empty_local_repo_test_async(|repo| async {
            // Make mutable copy so we can set remote
            let mut repo = repo;
            ...
            */
            ...
            // Create README
            let readme_path = repo.path.join("README.md");
            let readme_path = oxen_test::write_txt_file_to_path(readme_path, "README")?;
            repositories::add(&repo, &readme_path).await?;
            ...
            for i in 0..num_dirs {
                ...
                let file_path =
                    oxen_test::write_txt_file_to_path(file_path, format!("file -> {i}"))?;
            ...
            let file_path = oxen_test::write_txt_file_to_path(file_path, "modified file")?;
            repositories::add(&repo, &file_path).await?;
    ...
    async fn test_push_branch_with_with_no_new_commits() -> Result<(), OxenError> {
        oxen_test::run_training_data_repo_test_no_commits_async(|mut repo| async move {
            // Track a dir
    ...
    async fn test_cannot_push_two_separate_empty_roots() -> Result<(), OxenError> {
        oxen_test::run_no_commit_remote_repo_test(|remote_repo| async move {
            let ret_repo = remote_repo.clone();

            // Clone the first repo
            oxen_test::run_empty_dir_test_async(|first_repo_dir| async move {
                ...
                oxen_test::run_empty_dir_test_async(|second_repo_dir| async move {
                    ...
                    let new_file_path =
                        oxen_test::write_txt_file_to_path(new_file_path, "new file")?;
                    ...
                    let new_file_path =
                        oxen_test::write_txt_file_to_path(new_file_path, "new file 2")?;
                    ...
                    let new_file_path = second_cloned_repo.path.join(new_file);
                    let new_file_path =
                        oxen_test::write_txt_file_to_path(new_file_path, "new file 3")?;
                    repositories::add(&second_cloned_repo, &new_file_path).await?;
    ...
    async fn test_cannot_push_two_separate_repos() -> Result<(), OxenError> {
        oxen_test::run_training_data_repo_test_fully_committed_async(|mut repo_1| async move {
            oxen_test::run_training_data_repo_test_fully_committed_async(|mut repo_2| async move {
                // Add to the first repo
                let new_file = "new_file.txt";
                let new_file_path = repo_1.path.join(new_file);
                let new_file_path = oxen_test::write_txt_file_to_path(new_file_path, "new file")?;
                repositories::add(&repo_1, &new_file_path).await?;
                repositories::commit(&repo_1, "Adding first file path.")?;
                // Set/create the proper remote
                let remote = oxen_test::repo_remote_url_from(&repo_1.dirname());
                command::config::set_remote(&mut repo_1, constants::DEFAULT_REMOTE_NAME, &remote)?;
                oxen_test::create_remote_repo(&repo_1).await?;
                repositories::push(&repo_1).await?;

                // Adding two commits to have a longer history that also should fail
                let new_file = "new_file_2.txt";
                let new_file_path = repo_2.path.join(new_file);
                let new_file_path = oxen_test::write_txt_file_to_path(new_file_path, "new file 2")?;
                repositories::add(&repo_2, &new_file_path).await?;
                repositories::commit(&repo_2, "Adding second file path.")?;

                let new_file = "new_file_3.txt";
                let new_file_path = repo_2.path.join(new_file);
                let new_file_path = oxen_test::write_txt_file_to_path(new_file_path, "new file 3")?;
                repositories::add(&repo_2, &new_file_path).await?;
    ...
    async fn test_push_many_commits_default_branch() -> Result<(), OxenError> {
        oxen_test::run_many_local_commits_empty_sync_remote_test(
    ...
    async fn test_tree_cannot_push_two_separate_cloned_repos() -> Result<(), OxenError> {
        ...
        oxen_test::run_training_data_fully_sync_remote(|_, remote_repo_1| async move {
            let remote_repo_1_copy = remote_repo_1.clone();

            // Push the second repo with data
            oxen_test::run_training_data_fully_sync_remote(|_, remote_repo_2| async move {
                ...
                oxen_test::run_empty_dir_test_async(|first_repo_dir| async move {
                    ...
                    oxen_test::run_empty_dir_test_async(|second_repo_dir| async move {
                        ...
                        let first_remote =
                            oxen_test::repo_remote_url_from(&first_cloned_repo.dirname());
    ...
    async fn test_tree_cannot_push_when_remote_repo_is_ahead_same_file() -> Result<(), OxenError> {
        ...
        oxen_test::run_training_data_fully_sync_remote(|_, remote_repo| async move {
            ...
            oxen_test::run_empty_dir_test_async(|user_a_repo_dir| async move {
                ...
                // Clone Repo to User B
                oxen_test::run_empty_dir_test_async(|user_b_repo_dir| async move {
                    ...
                    let a_mod_file_path =
                        oxen_test::write_txt_file_to_path(a_mod_file_path, "I am the README now")?;
                    ...
                    let b_mod_file_path =
                        oxen_test::write_txt_file_to_path(b_mod_file_path, "I be the README now.")?;
                    ...
                    let b_mod_file_path = user_b_repo.path.join(mod_file);
                    let b_mod_file_path = oxen_test::write_txt_file_to_path(
                        b_mod_file_path,
            ...
            oxen_test::run_empty_dir_test_async(|user_a_repo_dir| async move {
                ...
                oxen_test::run_empty_dir_test_async(|user_b_repo_dir| async move {
                    ...
                    let modify_path_b = user_b_repo
                        ...
                        .join("train")
                        .join("annotations.txt");
                    oxen_test::write_txt_file_to_path(&modify_path_a, "new file")?;
                    ...
                    oxen_test::write_txt_file_to_path(&modify_path_b, "newer file")?;
        ...
        oxen_test::run_training_data_fully_sync_remote(|_, remote_repo| async move {
            ...
            oxen_test::run_empty_dir_test_async(|user_a_repo_dir| async move {
                ...
                oxen_test::run_empty_dir_test_async(|user_b_repo_dir| async move {
                    ...
                    // User A modifies
                    oxen_test::write_txt_file_to_path(&modify_path_a, "fancy new file contents")?;
                    repositories::add(&user_a_repo, &modify_path_a).await?;
                    let commit_a =
                    ...
                    repositories::add(&user_b_repo, &modify_path_b).await?;
                    // also add a file
                    // oxen_test::write_txt_file_to_path(&add_path_b, "new file")?;
...
",
    ...
    async fn test_push_subtree_nlp_classification() -> Result<(), OxenError> {
        ...
        oxen_test::run_training_data_fully_sync_remote(|_, remote_repo| async move {
            ...
            // Clone Repo
            oxen_test::run_empty_dir_test_async(|repos_base_dir| async move {
                ...
                let new_file_path =
                    oxen_test::write_txt_file_to_path(new_file_path, "image\tlabel")?;
    ...
    async fn test_push_partial_clone_nlp_classification() -> Result<(), OxenError> {
        ...
        oxen_test::run_training_data_fully_sync_remote(|_, remote_repo| async move {
            ...
            oxen_test::run_empty_dir_test_async(|repos_base_dir| async move {
                ...
                let new_file_path_1 =
                    oxen_test::write_txt_file_to_path(new_file_path_1, "image\tlabel1")?;
                ...
                let new_file_path_2 =
                    oxen_test::write_txt_file_to_path(new_file_path_2, "image\tlabel2")?;
                ...
                let modified_file_path =
                    oxen_test::write_txt_file_to_path(existing_file_path, "image\tmodified_label")?;
    ...
    async fn test_push_file_with_exact_avg_chunk_size() -> Result<(), OxenError> {
        oxen_test::run_readme_remote_repo_test(|local_repo, remote_repo| async move {
            ...
            oxen_test::run_empty_dir_test_async(|temp_dir| async move {
    ...
    async fn test_merge_conflict_push_failure() -> Result<(), OxenError> {
        ...
        oxen_test::run_training_data_fully_sync_remote(|original_repo, remote_repo| async move {
            let remote_repo_copy = remote_repo.clone();
            ...
            oxen_test::run_empty_dir_test_async(|clone_dir| async move {

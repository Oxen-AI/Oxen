...
mod tests {
    ...
    async fn test_command_restore_file_from_commit_id() -> Result<(), OxenError> {
        crate::test::run_empty_local_repo_test_async(|repo| async move {
            ...
            let hello_file = crate::test::modify_txt_file(hello_file, first_modification)?;
            repositories::add(&repo, &hello_file).await?;
            let first_mod_commit = repositories::commit(&repo, "Changing to spanish")?;

            // Modify again
            let second_modification = "Bonjour le monde";
            let hello_file = crate::test::modify_txt_file(hello_file, second_modification)?;
            repositories::add(&repo, &hello_file).await?;
    ...
    async fn test_restore_directory() -> Result<(), OxenError> {
        crate::test::run_training_data_repo_test_fully_committed_async(|repo| async move {
            ...
            let readme_path = crate::test::append_line_txt_file(readme_path, "Adding s'more")?;

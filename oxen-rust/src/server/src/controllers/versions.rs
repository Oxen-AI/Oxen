                ...
                Ok(data) => {
                    ...
                    match content_type {
                        ...
                        "application/zip" => {
                            ...
                            if let Err(e) = zip.start_file(file_hash, options).await {
                                log::error!("Failed to start zip file {file_hash}: {e}");
                                error_tx.send(OxenError::IO(e)).ok();
                                had_error = true;
                                break;
                            }

                            if let Err(e) = tokio::io::copy(&mut reader, &mut zip).await {
                        ...
                        _ => {
                            return Err(OxenError::BadRequest("Unsupported content type");)
                        }
                    }
                }
                Err(e) => {
...
=
                match zip.into_inner().await {
                    Ok(mut stream) => {
                        if let Err(e) = stream.shutdown().await {
                            log::error!("Failed to shutdown zip stream: {e}");
                            error_tx.send(OxenError::IO(e)).ok();
                            had_error = true;
                        }
                    }
                    Err(e) => {
                        log::error!("Failed to get stream from zip writer: {e}");
                        error_tx.send(OxenError::IO(e)).ok();
                        had_error = true;
                    }
                };

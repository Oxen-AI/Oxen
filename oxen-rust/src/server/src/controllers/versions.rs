...
pub async fn batch_upload(
    ...
    let namespace = path_param(&req, "namespace")?;
    let repo_name = path_param(&req, "repo_name")?;
    eprintln!("Getting local repo");
    let repo = get_repo(&app_data.path, namespace, &repo_name)?;
    eprintln!("Got local repo");
    ...
    while let Some(mut field) = payload.try_next().await? {
        ...
        if let Some(name) = content_disposition.get_name() {
            if name == "file[]" {
                ...
                let data_to_store =
                    match tokio::task::spawn_blocking(move || -> Result<Vec<u8>, OxenError> {
                        if is_gzipped {
                            log::debug!(
                ...
                eprintln!("Received decompressed data and storing into version store...");
                match version_store
                    .store_version(&upload_filehash, &data_to_store)
                    .await
                {
                    Ok(_) => {
                        eprintln!("Successfully stored version for hash: {}", &upload_filehash);

...
const TEST_REPO_PATH: &str = "/Users/josh/src/oxen/Oxen/watcher/oxen-rust/experiments/watcher-performance/test-repo";
...
fn benchmark_initial_scan(rt: &Runtime, results: &mut BenchmarkResults) {
    rt.block_on(async {
                    ...
                    if let Some(ft) = entry.file_type() {
                        if ft.is_file() {
                            if let Ok(metadata) = entry.metadata() {
                                ...
                                let _ = tx.lock().unwrap().send((entry.into_path(), Some(file_metadata)));
                            }
                        } else if ft.is_dir() {
        ...
        println!("  Scan found {} files and {} directories in {:.3}s", 
        ...
        println!("  Total initial scan time: {:.3}s", results.initial_scan_duration.as_secs_f64());
    });
}
...
fn benchmark_cache_updates(rt: &Runtime, results: &mut BenchmarkResults) {
    rt.block_on(async {
        ...
        cache.batch_update_new(initial_updates).await.unwrap();
        ...
        let start = Instant::now();
        cache.update_file(PathBuf::from("single_test.txt"), metadata).await.unwrap();
        results.single_update_duration = start.elapsed();
        println!("    Single update: {:.3}ms", results.single_update_duration.as_secs_f64() * 1000.0);
        ...
        let start = Instant::now();
        cache.batch_update_new(batch_100).await.unwrap();
        results.batch_100_update_duration = start.elapsed();
        println!("    Batch 100: {:.3}ms", results.batch_100_update_duration.as_secs_f64() * 1000.0);
        ...
        for i in 0..1000 {
            ...
            batch_1000.push((PathBuf::from(format!("batch1000_{}.txt", i)), Some(metadata)));
        }
        let start = Instant::now();
        cache.batch_update_new(batch_1000).await.unwrap();
        results.batch_1000_update_duration = start.elapsed();
        println!("    Batch 1000: {:.3}ms", results.batch_1000_update_duration.as_secs_f64() * 1000.0);
        ...
        for i in 0..10000 {
            ...
            batch_10000.push((PathBuf::from(format!("batch10000_{}.txt", i)), Some(metadata)));
        }
        let start = Instant::now();
        cache.batch_update_new(batch_10000).await.unwrap();
        results.batch_10000_update_duration = start.elapsed();
        println!("    Batch 10000: {:.3}ms", results.batch_10000_update_duration.as_secs_f64() * 1000.0);
    });
}
...
fn benchmark_tree_queries(rt: &Runtime, results: &mut BenchmarkResults) {
    rt.block_on(async {
        ...
        println!("    Full tree query: {:.3}ms", results.full_query_duration.as_secs_f64() * 1000.0);
        ...
        if let Some(dir) = sample_dir {
            ...
            println!("    Subtree query: {:.3}ms", results.subtree_query_duration.as_secs_f64() * 1000.0);
        }
    });
}
...
fn benchmark_serialization(rt: &Runtime, results: &mut BenchmarkResults) {
    rt.block_on(async {
        ...
        println!("    Serialization: {:.3}ms, size: {:.2} MB", 
        ...
        println!("    Protocol round-trip: {:.3}ms", 
...
fn benchmark_event_processing(rt: &Runtime, results: &mut BenchmarkResults) {
    rt.block_on(async {
        ...
        for i in 0..1000 {
            ...
            updates.push((PathBuf::from(format!("event_test_{}.txt", i)), Some(metadata)));
        }
        ...
        results.event_processing_rate = event_count as f64 / results.event_batch_processing_duration.as_secs_f64();
        
        println!("    Processing time: {:.3}ms", 
                 results.event_batch_processing_duration.as_secs_f64() * 1000.0);
        println!("    Processing rate: {:.0} events/second", results.event_processing_rate);
    });
}
...
fn measure_memory_usage(rt: &Runtime, results: &mut BenchmarkResults) {
    rt.block_on(async {
        ...
        println!("    Empty cache memory: ~{:.2} MB", results.memory_usage_empty as f64 / (1024.0 * 1024.0));
        ...
        println!("    After 100K files: ~{:.2} MB", results.memory_usage_after_scan as f64 / (1024.0 * 1024.0));
        ...
        for i in 0..10000 {
            ...
            more_updates.push((PathBuf::from(format!("mem_update_{}.txt", i)), Some(metadata)));
        }
        cache.batch_update_new(more_updates).await.unwrap();
        results.memory_usage_after_updates = estimate_memory_usage();
        println!("    After updates: ~{:.2} MB", results.memory_usage_after_updates as f64 / (1024.0 * 1024.0));
    });
}
...
fn print_results(results: &BenchmarkResults) {
    ...
    println!("  Serialization: {:.3}ms", results.tree_serialization_duration.as_secs_f64() * 1000.0);
    println!("  Size: {:.2} MB", results.tree_serialization_size as f64 / (1024.0 * 1024.0));
    println!("  Deserialization: {:.3}ms", results.tree_deserialization_duration.as_secs_f64() * 1000.0);
    println!("  Total round-trip: {:.3}ms", 
             (results.tree_serialization_duration + results.tree_deserialization_duration).as_secs_f64() * 1000.0);
    
    println!("\nEvent Processing Performance:");
    println!("  Batch processing time: {:.3}ms", results.event_batch_processing_duration.as_secs_f64() * 1000.0);
    println!("  Processing rate: {:.0} events/second", results.event_processing_rate);
    
    println!("\nMemory Usage:");
    println!("  Empty cache: ~{:.2} MB", results.memory_usage_empty as f64 / (1024.0 * 1024.0));
    println!("  After 100K files: ~{:.2} MB", results.memory_usage_after_scan as f64 / (1024.0 * 1024.0));
    println!("  After updates: ~{:.2} MB", results.memory_usage_after_updates as f64 / (1024.0 * 1024.0));
}
...
fn analyze_results(results: &BenchmarkResults) {
    ...
    let files_per_second = results.initial_scan_files_count as f64 / results.initial_scan_duration.as_secs_f64();
    let estimated_1m_scan = 1_000_000.0 / files_per_second;
    println!("\n  Estimated initial scan time for 1M files: {:.1}s", estimated_1m_scan);
    
    // Update performance analysis
    let update_throughput = 10000.0 / results.batch_10000_update_duration.as_secs_f64();
    println!("  Cache update throughput: {:.0} updates/second", update_throughput);
    ...
    println!("  Estimated serialized size for 1M files: {:.1} MB", estimated_1m_size);
    ...
    println!("  Estimated serialization time for 1M files: {:.2}s", estimated_1m_serialization);
    
    // Memory analysis
    let memory_per_file = (results.memory_usage_after_scan - results.memory_usage_empty) as f64 / 100000.0;
    let estimated_1m_memory = memory_per_file * 1_000_000.0 / (1024.0 * 1024.0);
    println!("  Estimated memory usage for 1M files: {:.1} MB", estimated_1m_memory);
    ...
    println!("  Batch update efficiency: {:.1}x faster than individual updates", 1.0 / batch_efficiency);
    ...
    if results.subtree_query_duration > Duration::ZERO {
        ...
        println!("  Full tree query is {:.1}x slower than subtree query", query_ratio);
    }
    ...
    if estimated_1m_scan < 10.0 {
        ...
        println!("  ⚠️  Initial scan may be slow for 1M files (>{:.0}s)", estimated_1m_scan);
    }
    ...
    if estimated_1m_memory < 500.0 {
        ...
        println!("  ⚠️  Memory usage may be high (>{:.0}MB for 1M files)", estimated_1m_memory);
    }

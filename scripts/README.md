...
Reshuffles a flat (or shallow) directory of files into a deeply nested, organically named directory tree. Designed to be used after `generate-dataset.py` to give synthetic data the kind of directory structure you'd find in a real project.
...
### `patch_dev_version.sh`

Patches `oxen-python`'s `pyproject.toml` and `Cargo.toml` with a PEP 440 dev version (e.g. `0.44.1.dev3`). Used by `publish_test_pypi.sh` and CI workflows to stamp pre-release builds without permanently changing the checked-in version.
...
Use `generate-dataset.py` to make the raw, randomized data files for your upload. Pay careful attention to the `--tier` parameter, which lets you make a segmented distribution for sampling file sizes.
Have this script output everything to a flat directory (which is the default behavior).

Then, use `reorganize-dataset.py` to generate a directory tree and move the generated files into it.

Finally, point `batch-workspace-upload.py` to that generated root directory to have it upload files in large batches using the workspace `add_files` function.

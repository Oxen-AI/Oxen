...
pub async fn transfer_namespace(
    ...
    repositories::transfer_namespace(&app_data.path, &name, &from_namespace, &to_namespace).await?;
    let repo =
        repositories::get_by_namespace_and_name(&app_data.path, &to_namespace, &name).await?.unwrap();
